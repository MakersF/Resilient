...
TEST(failures, ReturnMatches)
{
    ...
    auto value = equal3(f);
    EXPECT_TRUE(FailableTraits<decltype(value)>::isFailure(value));
}
...
TEST(failures, ReturnNotMatches)
{
    ...
    auto value = equal3(f);
    EXPECT_TRUE(FailableTraits<decltype(value)>::isSuccess(value));
}
...
TEST(failures, AnyNoMatches)
{
    ...
    EXPECT_TRUE(traits(value).isSuccess(value));
}
...
TEST(failures, AnyMatchesOne)
{
    ...
    auto f = []() { return make_failable<FailureExample>(3);};
    auto value = anyOf(equal3, equal4)(f);
    EXPECT_TRUE(traits(value).isFailure(value));
}
