...
public:
    ...
    Variant(Other&& other) : d_data(std::forward<Other>(other))
    {
    }
...
bool holds_alternative(
    Variant&& variant) // TODO make it fail to compile if the variant can not hold T
{
...
auto get(Variant&& variant) -> same_const_ref_as_t<Variant, T>
{
    ...
    // Put boost::strict_get in the scope and use the unscoped call so that the overload we defined
    // can be found
